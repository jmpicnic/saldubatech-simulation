...
import com.saldubatech.v1.equipment.units.shuttle.ShuttleLevelExecutor.{Groom, Inbound, Outbound}
...
//	  gw.shutdown()
	...
	val initialContents: Map[Aisle.LevelLocator, Material] = Map(
	...
	object outboundChannel extends DirectedChannel[Material](2, "R0"){
		def peekResource(): String = {
			...
			resources.sourceAssets.head
		}
	}
	val inboundChannel: DirectedChannel[Material] = DirectedChannel[Material](2, "L1")

	val underTest = ShuttleLevelExecutor(
		name = "underTest",
		physics,
		aisleLength = 7,
		inboundChannel,
		outboundChannel,
		Aisle.LevelLocator(Aisle.Side.LEFT, 0),
		initialContents)
...
//			equipmentObserver.testActor ! s"Restored channel: ${via.toString}"
	...
	val controllerActions: Seq[HarnessStep] = Seq(
		...
		//controllerReceivedLoadConfirm1,//3
		//controllerCompleteTask0,//4
