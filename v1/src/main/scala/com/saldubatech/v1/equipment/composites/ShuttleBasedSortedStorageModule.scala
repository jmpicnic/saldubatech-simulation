...
import com.saldubatech.v1.equipment.units.unitsorter.{CircularPathPhysics, CircularSorterExecution}
import com.saldubatech.util.Lang._

import scala.languageFeature.postfixOps

object ShuttleBasedSortedStorageModule {
	...
	class LinearTransportPhysics(val name: String, val delay: Long) {

	}
	case class EquipmentConfiguration(
		                                 nAisles: Int,
		                                 aisleInbounds: List[LinearTransportPhysics],
		                                 aisleOutbounds: List[LinearTransportPhysics],
		                                 //sorterGeography: TaggedGeography[String, ClosedPathPoint],
		                                 sorterPhysics: CircularPathPhysics,
		                                 nLevels: Int, // This should be moved to lift Physics
		                                 ioLevel: Int,
		                                 liftPhysics: CarriagePhysics,
		                                 aisleLength: Int, // This should be moved to shuttle Physics
		                                 shuttlePhysics: CarriagePhysics
	                                 )
}

abstract class ShuttleBasedSortedStorageModule(name: String,
                                      inboundChannels: List[DirectedChannel[Material]],
                                      outboundChannels: List[DirectedChannel[Material]],
                                      equipmentConfiguration: ShuttleBasedSortedStorageModule.EquipmentConfiguration,
                                      initialContents: Map[Int, Map[LevelLocator, Material]] = Map.empty)(implicit gw: Gateway)
	extends SimActorImpl(name, gw)
	{

		private def aisleBuilder(idx: Int,
		                         inboundChannel: DirectedChannel[Material],
		                         outboundChannel: DirectedChannel[Material]): ActorRef = {
			val aisleName = s"$name::aisle::$idx"
			gw.simActorOf(Props(new ShuttleAisleExecutor(aisleName,
				inboundChannel, outboundChannel, equipmentConfiguration.nLevels, equipmentConfiguration.liftPhysics,
				equipmentConfiguration.shuttlePhysics,equipmentConfiguration.aisleLength,initialContents,
				equipmentConfiguration.ioLevel)), aisleName)
		}

	//	private val sorterToAisleChannels: Vector[DirectedChannel[Material]]
	//	private val aisleToSorterChannels: Vector[DirectedChannel[Material]]

		private val aisles: List[ActorRef] = (0 until equipmentConfiguration.nAisles).map {
			idx => null//aisleBuilder(idx, sorterToAisleChannels(idx), aisleToSorterChannels(idx))
		} toList

		private val sorter: ActorRef = null/*gw.simActorOf(
			Props(new CircularSorterExecution(
				name ="sorter",
				aisleToSorterChannels, sorterToAisleChannels,
				sorterGeography,
				equipmentConfiguration.sorterPhysics)), name = "sorter")*/
		...
		override def configure: Configuring = {
			case ConfigureOwner(p_owner) =>
				_owner = p_owner.?
				gw.configure(sorter, ConfigureOwner(self))
				aisles.foreach(a => gw.configure(a, ConfigureOwner(self)))
		}
/*
