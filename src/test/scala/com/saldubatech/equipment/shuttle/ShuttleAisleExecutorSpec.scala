...
import com.saldubatech.equipment.shuttle.ShuttleAisleExecutor.{Groom, Inbound, Outbound}
...
//	  gw.shutdown()
	...
	val initialContents: Map[Int, Map[Aisle.LevelLocator, Material]] = Map(
		2 -> Map(
			Aisle.LevelLocator(Aisle.Side.LEFT, 3) -> materialL2_3,
			Aisle.LevelLocator(Aisle.Side.RIGHT, 6) -> materialR2_6),
		4 -> Map(
			Aisle.LevelLocator(Aisle.Side.LEFT, 5) -> materialL4_5,
			Aisle.LevelLocator(Aisle.Side.RIGHT, 2) -> materialR4_2)
	)

	object outboundChannel extends DirectedChannel[Material](2, "R0"){
		def peekResource(): String = {
			this.resources.sourceAssets.head
		}
	}
	val inboundChannel: DirectedChannel[Material] = DirectedChannel[Material](2, "L1")

	val underTest = ShuttleAisleExecutor(
		"underTest",
		inboundChannel,
		outboundChannel,
		nLevels = 6,
	...
	val storeCommand1 = Inbound(Aisle.Locator(3, Aisle.Side.RIGHT, 5))
	...
	val storeCommand2 = Inbound(Aisle.Locator(3, Aisle.Side.LEFT, 2))
...
//			equipmentObserver.testActor ! s"Restored channel: ${via.toString}"
	...
	val controllerActions: Seq[(SimActorMixIn, ActorRef, Long) => Processing] = Seq(
