...
import com.saldubatech.units.carriage.{Carriage, CarriageComponent, CarriageNotification}
...
object Shuttle2 {
	...
	case class Store(override val from: String, to: Carriage.SlotLocator) extends InboundCommand
	case class Retrieve(from: Carriage.SlotLocator, override val to: String) extends OutboundCommand
	case class LoopBack(override val from: String, override val to: String) extends InboundCommand with OutboundCommand
	case class Groom(from: Carriage.SlotLocator, to: Carriage.SlotLocator) extends ExternalCommand
	case class PutawayFromTray(to: Carriage.SlotLocator) extends ExternalCommand
	...
	case class InitialState(position: Int, inventory: Map[Carriage.SlotLocator, MaterialLoad])

		def dischargeSource[DownstreamMessageType >: ChannelConnections.ChannelDestinationMessage]
	(slot: Carriage.SlotLocator, manager: Processor.Ref, chOps: Channel.Ops[MaterialLoad, ShuttleSignal, DownstreamMessageType], host: Processor.Ref) = {
	...
	(configuration: Configuration[UpstreamMessageType, DownstreamMessageType],
	 initial: InitialState)(implicit clockRef: Clock.Ref, simController: SimulationController.Ref) = {
		...
		val domain = new Shuttle2(configuration.name, carriage, configuration, initial)
		new Processor[ShuttleSignal](configuration.name, clockRef, simController, domain.configurer)
	}
}

class Shuttle2[UpstreamSignal >: ChannelConnections.ChannelSourceMessage, DownstreamSignal >: ChannelConnections.ChannelDestinationMessage]
(name: String,
 shuttleProcessor: Processor[Carriage.CarriageSignal],
 configuration: Shuttle2.Configuration[UpstreamSignal, DownstreamSignal],
 initial: Shuttle2.InitialState) extends Identification.Impl(name) with CarriageComponent.Host[Shuttle2.ShuttleSignal, Shuttle2.ShuttleSignal] with LogEnabled {
	import Shuttle2._

	override type HOST_SIGNAL = ShuttleSignal

	sealed trait CarriageSignal extends ShuttleSignal
	case class Load(override val loc: Carriage.SlotLocator) extends CarriageComponent.LoadCmd(loc) with CarriageSignal
	case class Unload(override val loc: Carriage.SlotLocator) extends CarriageComponent.UnloadCmd(loc) with CarriageSignal
	case class Induct(override val from: Channel.End[MaterialLoad, ShuttleSignal], override val at: Carriage.SlotLocator)
		extends CarriageComponent.InductCmd[ShuttleSignal](from, at) with CarriageSignal
	case class Discharge(override val to: Channel.Start[MaterialLoad, ShuttleSignal], override val at: Carriage.SlotLocator)
		extends CarriageComponent.DischargeCmd[ShuttleSignal](to, at) with CarriageSignal

	override type HOST = Shuttle2[UpstreamSignal, DownstreamSignal]
	...
	override def inducter(from: Channel.End[MaterialLoad, ShuttleSignal], at: Carriage.SlotLocator) = Induct(from, at)
	override def discharger(to: Channel.Start[MaterialLoad, ShuttleSignal], at: Carriage.SlotLocator) = Discharge(to, at)
	...
	(inboundSlot: Carriage.SlotLocator,
	...
	private val inboundSlots: mutable.Map[String, Carriage.SlotLocator] = mutable.Map.empty
	...
	private val outboundSlots: mutable.Map[String, Carriage.SlotLocator] = mutable.Map.empty
	...
	private def verifyLocator(l: Carriage.SlotLocator): Option[Carriage.SlotLocator] = if(l.idx < configuration.depth && l.idx >= 0) Some(l) else None
	...
	private def rejectCommand(cmd: ExternalCommand, msg: String)(implicit ctx: CTX): RUNNER = {
		ctx.signal(manager, Shuttle2.NotAcceptedCommand(cmd, s"Unexpected Command: $cmd"))
		Processor.DomainRun.same
	}
