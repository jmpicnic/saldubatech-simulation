...
import com.saldubatech.v1.base.Material.{DefaultPalletBuilder, TotePallet}
...
import com.saldubatech.v1.ddes.{Gateway, SimActor}
import com.saldubatech.v1.equipment.units.grouping.TotePalletizer.GroupByNumber
...
class PalletizerOnePalletByNumberDelayComandSpec(_system: ActorSystem) extends BaseActorSpec(_system) {
	...
	def this() = this(ActorSystem("PalletizerOnePalletByNumberDelayComandSpec"))
	...
	val palletizeCmd = GroupByNumber(4)// No slot provided

	val controllerTrigger: HarnessTrigger = (host: SimActor, from: ActorRef, at: Long) => {
		...
		sendFirstLoads ~> upstreamEquipment now at
		palletizeCmd ~> underTest in (at -> 15)
	}
	...
	val staging3rdToteAndSend4thTote: HarnessStep = (host: SimActor, from: ActorRef, at: Long) => {
		...
		case StartTask(cmdId, initialTotes) if cmdId == palletizeCmd.uid && initialTotes == Seq(totes(0)) =>
	...
	"The Palletizer" should {
		"Create a pallet" when {
			"receiving 4 totes that fulfill a command" in {
				...
				upstreamObserver.expectMsgClass(classOf[AcknowledgeLoad[Material]])
				controllerObserver.expectMsg(StartTask(palletizeCmd.uid, Seq(totes(0))))
...
object PalletizerOnePalletByNumberDelayComandSpec {
	import SpecActorHarness._
